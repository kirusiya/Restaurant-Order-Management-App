...
// Almacenamiento temporal para las suscripciones de los usuarios.
// En una aplicación de producción, esto DEBERÍA almacenarse en una base de datos (ej. Supabase).
// const pushSubscriptions = []; // Eliminar esta línea
...
const { id } = req.params;
        ...
        if (orderItemsError) {
            console.error('Error checking for product in order items:', orderItemsError);
            return res.status(500).json({ error: 'Error al verificar la existencia del producto en órdenes.' });
        }

        if (orderItems && orderItems.length > 0) {
            // Obtener el nombre del producto para el mensaje de error
            ...
            const productName = productData ? productData.name : 'desconocido';
            console.warn(`Attempted to delete product ${id} (${productName}) which is part of an order.`);
            return res.status(409).json({ error: `No se puede eliminar el Producto "${productName}" porque está en una orden.` });
        }
...
const { id } = req.params;
// No se necesita parseInt(id) aquí si el ID es un UUID string
...
// if (isNaN(orderIdNum)) {
//   console.error('Invalid order ID for delete:', id);
//   return res.status(400).json({ error: 'ID de orden inválido.' });
// }

// Usamos supabaseAdmin (cliente con service_role) para eliminar órdenes
const { error } = await supabaseAdmin
.from('orders')
.delete()
.eq('id', id); // Usar el ID directamente como string
...
try {
    ...
    if (error) {
        // El error 23505 (violación de unicidad) ya no debería ocurrir si eliminamos primero,
        // a menos que haya una carrera de condiciones muy rápida o un problema con el endpoint.
        if (error.code === '23505') {
